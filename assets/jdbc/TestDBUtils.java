 ...
 * 
 * 2. BeanListHandler: 把结果集转为一个 Bean 的 List, 并返回. Bean 的类型在 创建
 * BeanListHanlder 对象时以 Class 对象的方式传入. 可以适应列的别名来映射 JavaBean 的属性名: String sql
 * = "SELECT id, name customerName, email, birth " + "FROM customers WHERE
 * id = ?";
 * 
...
public class TestDBUtils {
	...
	public void testQueryRunnerUpdate() {
		...
		String sql = "UPDATE customers SET cbalance = 500 WHERE cid IN (?,?)";
	...
	public void testResultSetHandler() {
		String sql = "SELECT student_id id, name, password, birth FROM students";
		...
		try {
			...
			/**
			 * 2. 调用 query 方法: ResultSetHandler 参数的作用: query 方法的返回值直接取决于
			...
			Object object = queryRunner.query(conn, sql, new ResultSetHandler() {
				...
				public Object handle(ResultSet rs) throws SQLException {
					...
					while (rs.next()) {
						...
						Student stu = new Student(id, name, password, birth);
						students.add(stu);
					}
					return students;
				}
	...
	/**
	 * 测试 ResultSetHandler 的 BeanListHandler 实现类 BeanListHandler: 把结果集转为一个 Bean
	...
	public void testBeanListHandler() {
		...
		try {
			...
			/*
			 * Student 类要存在无参数的构造器，否者会报错
			 */
			Object object = queryRunner.query(conn, sql, new BeanListHandler<>(Student.class));
	...
	public void testMapHandler() {
		...
		String sql = "SELECT student_id, name, password, birth FROM students WHERE student_id = ?";
	...
	public void testMapListHandler() {
		...
		String sql = "SELECT student_id, name, password, birth FROM students";
